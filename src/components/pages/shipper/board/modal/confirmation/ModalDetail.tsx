...
import { CombinedTransactionDetail } from '@/types/shipper/transaction';
...
interface ModalDetailProps {
  onClose: () => void;
  onSubmit: () => void;
  dataItem?: CombinedTransactionDetail;
}
...
const ModalDetail = ({ onClose, onSubmit, dataItem }: ModalDetailProps) => {
  const filteredOptions = TEMPERATURE_RANGE_LIST.slice(1).filter((option) =>
    dataItem?.request_snapshot?.temperature_range?.map((v) => String(v)).includes(option.value),
  );
  return (
    <>
      <CmnModalHeader
      ...
      <CmnModalBody classNames='gap-0'>
        <Item isModal dataItem={dataItem} />
        <div className='flex gap-8'>
          <div className='max-w-[21.875rem] w-full'>
            ...
            <div className='flex justify-between max-w-[16.9375rem] w-full items-center mt-[10px]'>
              ...
              {!dataItem ? (
                ...
                <span className='rounded-lg border border-default w-40 h-6 px-2 flex items-center justify-between'>
                  <span className='text-sm'>{dataItem.request_snapshot?.total_length}</span>
            ...
            <div className='flex justify-between max-w-[16.9375rem] w-full items-center mt-2'>
              ...
              {!dataItem ? (
                ...
                <span className='rounded-lg border border-default w-40 h-6 px-2 flex items-center justify-between'>
                  <span className='text-sm'>{dataItem.request_snapshot?.total_width}</span>
                  <p className='text-xs font-normal leading-[1.313rem]'>cm</p>
                </span>
              )}
            </div>
            <div className='flex justify-between max-w-[16.9375rem] w-full items-center mt-2'>
              ...
              {!dataItem ? (
                ...
                <span className='rounded-lg border border-default w-40 h-6 px-2 flex items-center justify-between'>
                  <span className='text-sm'>{dataItem.request_snapshot?.total_height}</span>
                  <p className='text-xs font-normal leading-[1.313rem]'>cm</p>
                </span>
              )}
            </div>
            <div className='flex justify-between max-w-[16.9375rem] w-full items-center mt-2'>
              ...
              {!dataItem ? (
                ...
                <span className='rounded-lg border border-default w-40 h-6 px-2 flex items-center justify-between'>
                  <span className='text-sm'>{dataItem.request_snapshot?.weight}</span>
            ...
            <CmnCheckboxGroup
              value={dataItem?.request_snapshot?.temperature_range?.map((v) => String(v))}
              size='sm'
              title=''
              isReadOnly
          ...
          <CmnTextarea
            ...
            title='コメント'
            readOnly
            defaultValue={!dataItem ? '' : dataItem.request_snapshot?.special_instructions?.toString()}
            maxLength={100}
          />
        </div>
      </CmnModalBody>
