...
const transportInfoInfoPostCSV = async ({ file }: { file: File }) => {
  const transportApi = transportService();
  const result = await transportApi.createTransportPlanCSV(file);
  return result;
};

const transportPlanDownloadCSV = async () => {
  const transportApi = transportService();
  return await transportApi.transportPlanDownloadCSV();
};
...
function PostCSV(props: PostCSVProps) {
    ...
    transportPlanDownloadCSV()
      .then((response: any) => {
        if (response && response?.status && response?.status !== 200) {
          ...
          link.href = url;
          link.setAttribute('download', TEMPLATE_CSV.TRANSPORT_PLAN);
      ...
      .catch((error) => {
        showError();
        console.log('[ERROR] === transportInfoDownloadCSV ====>:', error);
      })
      .finally(() => {
  ...
  const handleUploadCSV = () => {
    ...
    const showError = (error: any) => {
      console.log('[ERROR] ===== transportInfoPostCSV  ====> :', error);
      dispatch(
    ...
    setLoading(true);
    transportInfoInfoPostCSV({ file: files[0].file })
      .then((response) => {
        if (response && response?.status && response?.status !== 200) {
    ...
    <>
      {isOpen && (
        <CmnModal size='2xl' isOpen={isOpen} onClose={onClose}>
          <CmnModalHeader
            title='荷物情報の一括登録'
            description='一括登録テンプレートを使用して荷物情報の一括登録を行えます。'
          />
          <CmnModalBody>
