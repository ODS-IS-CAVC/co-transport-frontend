...
const cargoInfoPostCSV = async ({ file }: { file: File }) => {
  const cargoApi = cargoService();
  const result = await cargoApi.createCargoCSV(file);
  return result;
};

const cargoDownloadCSV = async () => {
  const cargoApi = cargoService();
  return await cargoApi.cargoDownloadCSV();
};
...
function PostCSV(props: PostCSVProps) {
  ...
  const handleUploadCSV = () => {
    ...
    const showError = (error: any) => {
      console.log('[ERROR] === cargoInfoPostCSV ====>:', error);
      dispatch(
    ...
    setLoading(true);
    cargoInfoPostCSV({ file: files[0].file })
      .then((response) => {
    ...
    cargoDownloadCSV()
      .then((response: any) => {
        if (response && response?.status && response?.status !== 200) {
          ...
          link.href = url;
          link.setAttribute('download', TEMPLATE_CSV.CARGO);
      ...
      .catch((error) => {
        showError();
        console.log('[ERROR] === cargoDownloadCSV ====>:', error);
      })
      .finally(() => {
