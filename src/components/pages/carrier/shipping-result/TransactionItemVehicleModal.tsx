...
import ConditionProposalVehicleModal from '../shipping-request/ConditionProposalVehicleModal';
...
function TransactionItemVehicleModal(props: TransactionItemModalProps) {
  const {
    id,
    isOpen = false,
    item,
  ...
  const [selectedItem, setSelectedItem] = useState<ProposeTrspPlan | null>(null);
  ...
  return (
    <>
      <CmnModal id={id} isOpen={isOpen} placement='top' size='5xl' onClose={onClose} radius='none'>
        <CmnModalHeader
          title={`予約可能な運行 - ${item.length}件`}
          description={`マッチングしていない空便情報（運送能力）に予約可能な運行です。`}
          className='flex flex-col font-normal'
        />
        <CmnModalBody classNames='text-sm leading-normal'>
          <p className='text-base word-break'>
            ※キャリア間マッチングで予約可能な条件は要検討。案:運賃(カットオフ運賃)。例えば、運賃は+-5,000円の範囲で提案可能(カットオフ運賃は適宜調整可能)、それぞれに該当しないものは提案不可。
            <br />
            ※予約可能な運行は、トレーラーが後空きの場合のみとする？
            前後埋まっているものをバラしてキャリア間マッチングさせるかどうかは要確認。
          </p>
          {item.map((data: ProposeTrspPlan) => {
            const truck =
              data?.trsp_plan_line_item &&
              data?.trsp_plan_line_item.length > 0 &&
              data?.trsp_plan_line_item[0].car_info?.find((trk) => trk.tractor_idcr === '1');
            const trailers =
              data?.trsp_plan_line_item &&
              data?.trsp_plan_line_item.length > 0 &&
              data?.trsp_plan_line_item[0].car_info
                ?.filter((car) => car.tractor_idcr !== '1')
                .sort((a, b) => Number(a.display_order) - Number(b.display_order));
            return (
              <Card
                key={`${data.id}-${parent.id}`}
                className='p-4 border border-gray-border rounded-lg mt-4 mx-4'
                shadow='none'
              >
                <CardHeader className={`text-sm pb-0 p-2 gap-2 flex justify-between`}>
                  <div className='flex text-foreground text-sm items-center'>
                    ...
                    <p className='text-2xl font-semibold whitespace-nowrap flex'>
                      {data.trsp_plan_line_item &&
                        data.trsp_plan_line_item.length > 0 &&
                        data.trsp_plan_line_item[0].service_name}
                    </p>
                  </div>
                  <div className='flex text-foreground text-base'>
                    ...
                    <span className='whitespace-nowrap flex'>
                      ...
                      <p>{data.service_strt_date && dayjs(data.service_strt_date).format(DATE_FORMAT.JAPANESE)}</p>
                    </span>
                  </div>
                </CardHeader>
                  ...
                  <div className='flex'>
                    <div className='flex items-center px-3 gap-3 border border-default rounded-lg'>
                      ...
                      <p className='text-2xl font-medium'>
                        {`${getPrefectureName(regions, data.departure_from)} ${
                          (data.service_strt_time &&
                            dayjs(data.service_strt_time, TIME_FORMAT.HH_MM).format(TIME_FORMAT.HH_MM)) ||
                          '--:--'
                        }発`}
                      </p>
                      ...
                      <p className='text-2xl font-medium'>
                        {`${getPrefectureName(regions, data.arrival_to)} ${
                          (data.service_end_time &&
                            dayjs(data.service_end_time, TIME_FORMAT.HH_MM).format(TIME_FORMAT.HH_MM)) ||
                          '--:--'
                        }着`}
                      </p>
                    </div>
                              ...
                              <div className='flex items-center gap-2 w-full mb-1'>
                                {!car.order_status ? (
                                  <>
                                    <Chip
                                      className={
                                        'h-8 px-2 py-1 text-base text-white bg-[#259d63] rounded-lg justify-center items-center gap-2 inline-flex'
                                      }
                                      size='lg'
                                      radius='sm'
                                    >
                                    ...
                                    <div className='col-span-1 flex flex-col justify-end items-end space-y-2'>
                                      <Button
                                        className='text-neutral w-[13rem] font-bold text-base'
                                        size='sm'
                                        color='primary'
                                        radius='sm'
                                        onPress={() => {
                                          ...
                                          fetchCutOffInfos(parent.vehicle_availability_resource_id);
                                        }}
                                      >
                                        条件を調整して予約する
                                      </Button>
      ...
      {selectedItem && selectedCar && (
        <ConditionProposalVehicleModal
          id={`condition-proposal-vehicle-modal-${selectedItem?.id}`}
          parent={parent}
          item={selectedItem!}
          car={selectedCar!}
...
export default TransactionItemVehicleModal;
